...
class UNetVitVer02Config(BaseModelConfig):
    bias: bool
    input_channels: int
    feat_channels_0: int
    feat_channels_1: int
    ...
    lr_y_size: int
    lr_z_size: int
    use_vit_skip_connect: bool = False
...
class VitEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        dropout: float,
        num_block: int,
        use_vit_skip_connect: bool,
    ):
        #
        super().__init__()

        self.use_vit_skip_connect = use_vit_skip_connect
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = self.encoder(identity)

        if self.use_vit_skip_connect:
            y = y + identity
...
class UNetVitVer02(nn.Module):
    def __init__(
        self,
        cfg: UNetVitVer02Config,
    ):
        #
        super().__init__()
        ...
        logger.info("U-Net ViT Ver02 is initialized")
        ...
        self.latent_feature_extractor = VitEncoder(
            ...
            use_vit_skip_connect=self.cfg.use_vit_skip_connect,
        )
