...
class UNetVitVer03Config(BaseModelConfig):
    bias: bool
    input_channels: int
    feat_channels_0: int
    feat_channels_1: int
...
class Encoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return identity, enc1, enc2, bottleneck
...
class UNetVitVer03(nn.Module):
    def __init__(
        self,
        cfg: UNetVitVer03Config,
    ):
        #
        super().__init__()
        ...
        logger.info("U-Net ViT Ver03 is initialized")
        ...
        self.latent_2d_space_shape = [
            size // self.scale_factor**3 for size in self.init_space_shape[1:]
        ]

        self.latent_3d_space_shape = [
            size // self.scale_factor**3 for size in self.init_space_shape
        ]
        ...
        # ViT Encoder
        # channel and depth dim is flattened
        # That is because self.latent_3d_space_shape[0] is multiplied in in_channel
        self.latent_feature_extractor = VitEncoder(
            in_channels=self.cfg.encoder_output_channels * self.latent_3d_space_shape[0],
        ...
        self.ffn = ResidualFFN(
            ...
            * self.latent_3d_space_shape[0]
            * self.latent_3d_space_shape[1]
            * self.latent_3d_space_shape[2],
        ...
        self.decoder = Decoder(
            ...
            feat_channels_1=self.cfg.feat_channels_2,
            feat_channels_2=self.cfg.feat_channels_1,
    ...
    def forward(self, lr_pv: torch.Tensor, hr_obs: torch.Tensor) -> torch.Tensor:
        ...
        # Encoder block
        identity, enc1, enc2, bottleneck = self.encoder(feat)

        # Reshape by flattening channel and depth dim for ViT
        ...
        # Decoder block
        y = self.decoder(identity, enc1, enc2, bottleneck)
