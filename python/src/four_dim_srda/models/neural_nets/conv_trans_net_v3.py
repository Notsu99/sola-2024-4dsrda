...
class ConvTransNetVer03Config(BaseModelConfig):
    bias: bool
    feat_channels_0: int
    feat_channels_1: int
    feat_channels_2: int
...
class ConvTransNetVer03(nn.Module):
    def __init__(
        self,
        config_ins: ConvTransNetVer03Config,
    ):
        super().__init__()
        ...
        # 8 == 2**3 (encoder has 3 blocks to down sample)
        self.latent_x_size = self.cfg.hr_x_size // 8
        self.latent_y_size = self.cfg.hr_y_size // 8
        self.latent_z_size = self.cfg.hr_z_size // 8
        self.latent_dim = (
            self.latent_x_size
            * self.latent_y_size
            * self.latent_z_size
