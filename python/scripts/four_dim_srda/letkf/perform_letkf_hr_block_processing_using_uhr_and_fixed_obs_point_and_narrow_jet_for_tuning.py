...
from src.four_dim_srda.utils.calc_statistics import calc_maer
from src.four_dim_srda.utils.enkf_helper import get_obs_matrix, get_uhr_pvs
...
if __name__ == "__main__":
    try:
        ...
        result_dir = f"{RESULT_DIR}/{experiment_name}/letkf/perform_letkf_hr_using_uhr/tuning/use_narrow_jet/{og_val}/{na_val}_{config_name}"
        os.makedirs(result_dir, exist_ok=True)

        log_dir = f"{LOG_DIR}/{experiment_name}/letkf/perform_letkf_hr_using_uhr/tuning/use_narrow_jet/{og_val}/{na_val}_{config_name}"
        ...
        for i_seed_uhr in range(i_seed_uhr_start, cfg_cfd.seed_config.uhr_seed_end + 1):
            ...
            #
            obs_npz_file_path = f"{RESULT_DIR}/{experiment_name}/srda/{model_name}/use_narrow_jet/{config_srda_name}/UHR_seed_{i_seed_uhr:05}.npz"
        ...
        # Keep only the data for the analysis times
        all_letkf_means = all_letkf_means[:, :: cfg_cfd.da_config.forecast_span, ...]
        ...
        # Calc & Save statistics
        if (
            len(all_letkf_means)
            == cfg_cfd.seed_config.uhr_seed_end - cfg_cfd.seed_config.uhr_seed_start + 1
        ):
            #
            all_uhr_gt = get_uhr_pvs(
                result_dir_path=UHR_DATA_DIR,
                num_times=NUM_TIMES,
                cfg=cfg_cfd,
            )
            # Keep only the data for the analytical times
            all_uhr_gt = all_uhr_gt[:, :: cfg_cfd.da_config.forecast_span, ...]

            # interpolation
            all_letkf_uhr_fcst = []
            for ib in range(len(all_letkf_means)):
                _tmp = interpolate_2d(
                    data=all_letkf_means[ib],
                ...
                all_letkf_uhr_fcst.append(_letkf_uhr_fcsts)

            # Stack along the simulation dim
            all_letkf_uhr_fcst = torch.stack(all_letkf_uhr_fcst, dim=0)
            assert all_letkf_uhr_fcst.shape == all_uhr_gt.shape

            # Calc maer
            maer = calc_maer(all_gt=all_uhr_gt, all_fcst=all_letkf_uhr_fcst)
            maer_time_avg = torch.mean(maer, dim=0)

            output_npz_file_path = f"{result_dir}/maer.npz"
            np.savez(
                output_npz_file_path,
                maer=maer,
                maer_time_avg=maer_time_avg,
            )
