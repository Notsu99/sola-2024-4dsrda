...
if __name__ == "__main__":
    try:
        ...
        log_dir = f"{LOG_DIR}/{experiment_name}/evaluation/{model_name}/use_narrow_jet/store_only_forecast/{cfg_srda_name}"
        result_dir = f"{RESULT_DIR}/{experiment_name}/srda/{model_name}/use_narrow_jet/store_only_forecast/{cfg_srda_name}"
        ...
        UHR_DATA_DIR = (
            f"{DATA_DIR}/{experiment_name}/cfd_simulation/qg_model/uhr_pv_narrow_jet"
        )
        ...
        for i_seed_uhr in range(
            ...
            for i_cycle in tqdm(range(NUM_TIMES)):
                ...
                if i_cycle > 0 and i_cycle % ASSIMILATION_PERIOD == 0:
                    ...
                    # And this test stores only forecast
                    # So NaN values also are substituted for first current analysis
                    if len(srda_forecast) == 0:
                        ...
                        dummy_lr = torch.full(
                            size=(
                                lr_forecast.shape[0],
                                ASSIMILATION_PERIOD // OUTPUT_SCALE_FACTOR + 1,
                            )
                            + lr_forecast.shape[2:],
                            fill_value=torch.nan,
                            dtype=srda.dtype,
            ...
            # srda_forecast has the last forecast value
            # Therefore, time dim becomes NUM_TIMES + 1
            assert srda_forecast.shape == (
                NUM_TIMES + 1,
                cfg_cfd.hr_base_config.nz,
                cfg_cfd.hr_base_config.ny,
                cfg_cfd.hr_base_config.nx,
