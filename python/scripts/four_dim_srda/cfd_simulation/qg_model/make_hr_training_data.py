...
def write_out(
    *,
    ...
    for i_batch in range(n_batch):
        file_path = f"{result_dir_path}/seed{i_seed:05}_start{out_step_start:03}_end{out_step_end:03}_hr_pv_{i_batch:02}.npy"
        np.save(file_path, hr_pv[i_batch].numpy())
...
if __name__ == "__main__":
    try:
        ...
        result_dir = f"{RESULT_DIR}/{experiment_name}/{SIMULATION_NAME}/hr_pv"
        log_dir = f"{LOG_DIR}/{experiment_name}/{SIMULATION_NAME}/hr_pv/{config_name}"
        ...
        model = QGModel(cfg.hr_base_config)

        logger.info("\n*********************************************************")
        logger.info("Start simulation")
        logger.info("*********************************************************\n")

        for i_seed in range(cfg.seed_config.seed_start, cfg.seed_config.seed_end + 1):
            ...
            if cfg.jet_profile == "tanh":
                q0 = make_jet_pv_with_tanh_profile(cfg.hr_base_config)
                model.initialize_pv(q0)
            elif cfg.jet_profile == "linear":
                q0 = make_jet_pv_with_linear_profile(cfg.hr_base_config)
                model.initialize_pv(q0)
            elif cfg.jet_profile == "sech-squared":
                u = make_jet_velocity_with_sech_squared_profile(cfg.hr_base_config)[
                    None, ...
                ]
                noise = cfg.hr_base_config.noise_amplitude * torch.randn(
                    size=(cfg.hr_base_config.n_batch,) + u.shape[-3:]
                )
                model.initialize(grid_u=u, grid_pv_noise=noise)
            elif cfg.jet_profile == "sech_squared_and_sigmoid":
                u = make_jet_velocity_with_sech_squared_and_sigmoid_profile(cfg.hr_base_config)[
                    None, ...
                ]
                noise = cfg.hr_base_config.noise_amplitude * torch.randn(
                    size=(cfg.hr_base_config.n_batch,) + u.shape[-3:]
            ...
            output_tsteps = torch.arange(
                ...
                cfg.time_config.end_time + cfg.time_config.output_hr_dt,
                cfg.time_config.output_hr_dt,
            )

            for t in tqdm(output_tsteps):
                logger.debug(f"t = {t} start.")

                n_steps = int(cfg.time_config.output_hr_dt / cfg.time_config.hr_dt)

                model.integrate_n_steps(
                    dt_per_step=cfg.time_config.hr_dt,
                    n_steps=n_steps,
                )
            ...
            # dim = batch, time, z, y, x

            logger.info(f"hr_pv shape = {qs.shape}, data num along time = {len(ts)}\n")
