...
def write_out(
    *,
    ...
    file_path = f"{result_dir_path}/seed{i_seed:05}_start{out_step_start:03}_end{out_step_end:03}_lr_pv.npy"
    np.save(file_path, lr_pv.numpy())
...
if __name__ == "__main__":
    try:
        ...
        result_dir = f"{RESULT_DIR}/{experiment_name}/{SIMULATION_NAME}/lr_pv_narrow_jet"
        log_dir = f"{LOG_DIR}/{experiment_name}/{SIMULATION_NAME}/lr_pv_narrow_jet/{config_name}"
        ...
        # Change batch num because only one result is needed in test simulation
        cfg.lr_base_config.n_batch = 1
        model = QGModel(cfg.lr_base_config)
        ...
        for i_seed in range(
            ...
            if cfg.jet_profile == "tanh":
                q0 = make_jet_pv_with_tanh_profile(cfg.lr_base_config)
            elif cfg.jet_profile == "linear":
                q0 = make_jet_pv_with_linear_profile(cfg.lr_base_config)
            elif cfg.jet_profile == "sech-squared":
                u = make_jet_velocity_with_sech_squared_profile(
                    cfg.lr_base_config, use_shift_y=False, use_narrow_jet=True
                )[None, ...]
                noise = cfg.lr_base_config.noise_amplitude * torch.randn(
                    size=(cfg.lr_base_config.n_batch,) + u.shape[-3:]
                )
            ...
            output_tsteps = torch.arange(
                cfg.time_config.start_time + cfg.time_config.output_lr_dt,
                cfg.time_config.end_time + cfg.time_config.output_lr_dt,
                cfg.time_config.output_lr_dt,
            )
            ...
            for t in tqdm(output_tsteps):
                ...
                model.integrate_n_steps(
                    dt_per_step=cfg.time_config.lr_dt,
                    n_steps=n_steps,
                )
            ...
            logger.info(f"lr_pv shape = {qs.shape}, data num along time = {len(ts)}\n")
